...
public class SegTag {
	 ...
	 * @return
	 */
	public SegResult split(String src, Boolean withTag) {
				...
				if (sen.isSeg()) {
					...
					for (ArrayList<Integer> onePath : bipath) {
						...
						for (ArrayList<Integer> optOnePath : optBipath) {
							...
							String adjrs = withTag ? outputResult(adjResult)
									: outputResultNoTag(adjResult);
	...
	public static void main(String[] args) {
		...
		try {
			while ((line = reader.readLine()) != null) {
				try {
					SegResult seg_res = segTag.split(line, true);
